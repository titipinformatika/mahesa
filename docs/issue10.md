# 📋 Eksekusi Issue #10: Seeder Database Master

Dokumen ini adalah panduan **Copy-Paste** (Sangat Detail) untuk menyelesaikan [Issue #10]. Jika Anda menggunakan AI, berikan seluruh teks di bawah ini ke AI Anda. Jika Anda *Programmer Junior*, cukup ikuti langkahnya satu per satu.
...
Sebuah skrip otomatis (`src/db/seed.ts`) yang mengisi database dengan data awal (**dummy data**) agar Frontend dan Mobile developer bisa langsung menguji integrasi tanpa harus membuat data manual lewat Postman.

### Kenapa penting?
- Tim Frontend butuh data nyata untuk menampilkan UI (daftar pegawai, pohon organisasi, dll).
- Tim Mobile butuh akun test untuk login.
- Seeder yang repeatable memudahkan reset database saat pengembangan.

### Skema Database yang Sudah Ada

Berikut tabel-tabel yang perlu diisi oleh seeder (sesuai urutan dependensi):

| Urutan | Tabel | File Skema | Keterangan |
|--------|-------|------------|------------|
| 1 | `level_unit_kerja` | `src/db/schema/organisasi.ts` | Level hierarki: Dinas (1), UPT (2), Sekolah (3) |
| 2 | `dinas` | `src/db/schema/organisasi.ts` | Data dinas utama |
| 3 | `unit_kerja` | `src/db/schema/organisasi.ts` | Unit-unit di bawah dinas. **Wajib** punya `id_dinas`, `id_level_unit`, `latitude`, `longitude` |
| 4 | `skema_jam_kerja` | `src/db/schema/pegawai.ts` | Jam kerja standar (misal: 07:30 - 16:00) |
| 5 | `master_jenis_kepegawaian` | `src/db/schema/pegawai.ts` | Jenis: PNS, PPPK, Honorer, dll |
| 6 | `pengguna` | `src/db/schema/pegawai.ts` | Akun login (email + password hash) |
| 7 | `pegawai` | `src/db/schema/pegawai.ts` | Data pegawai lengkap. **Wajib** punya `id_pengguna`, `id_unit_kerja`, `nik`, `nama_lengkap`, `jenis_kelamin`, `tanggal_masuk` |
| 8 | `pejabat_unit_kerja` | `src/db/schema/organisasi.ts` | Pimpinan unit kerja |

### Catatan Penting tentang Foreign Key
- `unit_kerja.id_dinas` → referensi ke `dinas.id`
- `unit_kerja.id_level_unit` → referensi ke `level_unit_kerja.id`
- `unit_kerja.id_induk_unit` → referensi ke `unit_kerja.id` (opsional, untuk hierarki UPT → Sekolah)
- `pegawai.id_pengguna` → referensi ke `pengguna.id`
- `pegawai.id_unit_kerja` → referensi ke `unit_kerja.id`
- `pegawai.id_skema_jam_kerja` → referensi ke `skema_jam_kerja.id` (opsional)
- `pejabat_unit_kerja.id_pegawai` → referensi ke `pegawai.id`
- `pejabat_unit_kerja.id_unit_kerja` → referensi ke `unit_kerja.id`

**PENTING:** Insert data HARUS mengikuti urutan di atas. Jika tidak, akan terjadi error `foreign key constraint violation`.

---

## 🛠️ Langkah 1: Buat File Seeder

Buat file baru di `backend/src/db/seed.ts` dan masukkan kode berikut:
...
## 🛠️ Langkah 2: Tambahkan Script di `package.json`

Buka file `backend/package.json`, cari bagian `"scripts"` dan tambahkan baris baru:
...
✅ SEEDING SELESAI!
```
Dan tabel akun serta hierarki organisasi ditampilkan.

### Verifikasi Login
Jalankan server (`bun run dev`) lalu coba login di Postman:
```
...
```
→ Expect `200` dengan token JWT.

### Verifikasi Data
```
GET http://localhost:3000/v1/pegawai?page=1&limit=10
Header: Authorization: Bearer <token>
```
→ Expect `200` dan daftar 5 pegawai.

```
GET http://localhost:3000/v1/pohon-organisasi
Header: Authorization: Bearer <token>
```
→ Expect `200` dan hierarki Dinas → UPT → Sekolah.

Jika semua verifikasi berjalan sesuai ekspektasi, **Issue #10 dinyatakan SELESAI**.
